...
export default function Contactanos() {
  ...
  const [errors, setErrors] = useState({ email: "" }); // nuevo estado para errores
  ...
  const onChange = (e) => {
    ...
    if (name === "email") {
      if (!value.includes("@")) {
        ...
        setErrors((prev) => ({ ...prev, email: "Formato de correo no válido (ej: nombre@dominio.com)" }));
      } else {
  ...
  return (
    <Box sx={{ bgcolor: "#F5F6F8", minHeight: "100vh" }}>
      {/* Hero */}
      <Box sx={{ width: "100%", bgcolor: "#0B3D91", color: "#FFFFFF", py: { xs: 4, md: 6 }, textAlign: "center", px: 2 }}>
        <Typography variant="h4" sx={{ fontWeight: 700, lineHeight: 1.2, fontSize: { xs: "1.8rem", md: "2rem" } }}>
          Contáctanos
        </Typography>
        <Typography variant="body1" sx={{ mt: 1.5, maxWidth: 700, mx: "auto", color: "rgba(255,255,255,0.8)", fontSize: { xs: "0.95rem", md: "1rem" } }}>
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 }, display: "flex", justifyContent: "center" }}>
        <Paper elevation={0} sx={{
          ...
          p: { xs: 3, md: 4 }, bgcolor: "#FFFFFF", width: "100%", maxWidth: "700px", mx: "auto", textAlign: "center",
        }}>
          <Typography variant="h6" sx={{ fontWeight: 600, color: "#1E1E1E", fontSize: { xs: "1.05rem", md: "1.15rem" }, mb: 2 }}>
            Envíanos un mensaje
          </Typography>
          ...
          <Box component="form" noValidate onSubmit={handleSubmit}
            ...
            <Stack spacing={2}>
              <TextField fullWidth required label="Nombre" name="name" value={form.name} onChange={onChange}
                         variant="outlined" size="medium" InputLabelProps={{ sx: { fontSize: "0.9rem" } }} />
              ...
              <TextField fullWidth label="Asunto" name="subject" value={form.subject} onChange={onChange}
                         variant="outlined" size="medium" InputLabelProps={{ sx: { fontSize: "0.9rem" } }} />

              <TextField fullWidth required label="Mensaje" name="message" value={form.message} onChange={onChange}
                         placeholder="Cuéntanos cómo podemos ayudarte…" variant="outlined" multiline minRows={6}
                         InputLabelProps={{ sx: { fontSize: "0.9rem" } }} />

              <Box sx={{ display: "flex", justifyContent: "center", mt: 1 }}>
                <Button type="submit" variant="contained" disableElevation disabled={sending}
...
const iconBtnSx = {
  ...
  bgcolor: "#FFFFFF", boxShadow: "0 2px 4px rgba(0,0,0,0.04), 0 1px 2px rgba(0,0,0,0.04)",
